...
app.get('/user', jwtValidation, async (req, res) => {
    const { idUsuario } = res.locals;
...
app.post('/contact', jwtValidation, (req, res) => {
    ...
    const { nome, stack, telefones, email, endereco, notas, foto = imgUserDafault } = req.body;
    const { idUsuario } = res.locals;
...
app.patch('/contact', jwtValidation, (req, res) => {
    ...
    const { idContato, nome, stack, telefones, email, endereco, notas, foto } = req.body;
    const { idUsuario } = res.locals;

    try {
        const dados = { id: idContato, nome, stack, telefones, email, endereco, notas, foto };
